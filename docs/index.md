# World Model Blueprint

This blueprint is an operating system for moving from enterprise autonomy data to a fine-tuned world model and a governed sim-to-real loop. It is built for organisations whose advantage lives in the physical world: surgical robotics, agricultural machinery, industrial automation, autonomous vehicles, mobile robots, and smart infrastructure.

![World Model Blueprint: autonomous robots, digital twins, and governed world-model data flows](assets/images/world-model-blueprint-social-preview.jpg)

## Read first

- [Quickstart](quickstart.md)
- [Blueprint](blueprint.md)
- [Reference architecture](reference-architecture.md)
- [Source map](source-map.md)

## Implementation guidance

- [Data strategy](data-strategy.md)
- [Post-training](post-training.md)
- [Infrastructure and execution](infrastructure.md)
- [Deployment](deployment.md)
- [Agent system](agent-system.md)
- [Governance](governance.md)
- [Toolchain](toolchain.md)
- [Manifest contracts](manifest-contracts.md)
- [Benchmarks](benchmarks.md)
- [Repository structure](repository-structure.md)

## Architecture assets

- [Architecture diagram](assets/architecture.svg)
- [Editable Mermaid diagram](assets/architecture.mmd)
- [Data and SimReady factory diagram](assets/data-factory.svg)
- [Neural asset services diagram](assets/neural-asset-services.svg)
- [Agent workflow diagram](assets/agent-workflow.svg)
- [Execution lane selection diagram](assets/execution-lanes.svg)
- [Inference services diagram](assets/inference-services.svg)
