# Operating playbook: Autonomy data strategist

## Role in the blueprint

Role: turn an open-ended autonomy ambition into a precise model, data, simulation, safety, and deployment brief.
...
2. **Evidence**: list the records already available and the records that are required by `schemas/programme-intake-manifest.schema.json`.
...
```bash
scripts/wmb new-manifest --schema schemas/programme-intake-manifest.schema.json --output artifacts/autonomy-data-strategist.json
scripts/wmb validate-manifest --schema schemas/programme-intake-manifest.schema.json --manifest artifacts/autonomy-data-strategist.json
