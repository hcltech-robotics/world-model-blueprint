---
name: governance-provenance-lead
description: >-
...
Use this skill to make the end-to-end world-model capability defensible to engineering, safety, regulatory, legal, and operations leaders. It owns the decision record for its stage of the data-to-world-model-to-sim2real pipeline and must emit artefacts that downstream agents can validate.

## Trigger conditions

Activate this skill when the request involves any of the following:

- Governance record and audit evidence map.
- Inputs such as license records, data manifests, model manifests, evaluation decisions, deployment records.
...
| Tool | Use |
| --- | --- |
| `scripts/wmb new-manifest --schema schemas/governance-record.schema.json --output <path>` | Create the stage manifest contract. |
| `scripts/wmb validate-manifest --schema schemas/governance-record.schema.json --manifest <path>` | Validate required fields and basic types. |
...
```bash
bash skills/governance-provenance-lead/scripts/run-checks.sh <manifest-path>
```
...
2. Locate the relevant manifest, or create a skeleton from `schemas/governance-record.schema.json`.
...
1. Map every artefact to owner, permitted use, retention, privacy, export-control, and safety classification.
2. Trace source data to dataset versions, training runs, checkpoints, evaluations, services, and incident records.
3. Check that agent-generated artefacts are reviewed before use in training, deployment, or safety acceptance.
4. Ensure repository content excludes secrets, protected data, gated weights, and operational identifiers.
5. Require release records for signatures, scans, benchmarks, approvals, and promotion decisions.
...
- Required manifest fields validate against `schemas/governance-record.schema.json`.
