# Operating playbook: SimReady content integration

## Role in the blueprint

Role: turn physical differentiators into simulation-ready assets and scenario evidence for world-model training and evaluation.
...
2. **Evidence**: list the records already available and the records that are required by `schemas/simready-asset-manifest.schema.json`.
...
```bash
scripts/wmb new-manifest --schema schemas/simready-asset-manifest.schema.json --output artifacts/simready-content-integration.json
scripts/wmb validate-manifest --schema schemas/simready-asset-manifest.schema.json --manifest artifacts/simready-content-integration.json
