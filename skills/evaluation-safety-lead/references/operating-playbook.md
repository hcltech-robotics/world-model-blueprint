# Operating playbook: Evaluation safety lead

## Role in the blueprint

Role: separate model quality from deployment readiness and make promotion evidence reproducible.
...
2. **Evidence**: list the records already available and the records that are required by `schemas/evaluation-manifest.schema.json`.
...
```bash
scripts/wmb new-manifest --schema schemas/evaluation-manifest.schema.json --output artifacts/evaluation-safety-lead.json
scripts/wmb validate-manifest --schema schemas/evaluation-manifest.schema.json --manifest artifacts/evaluation-safety-lead.json
