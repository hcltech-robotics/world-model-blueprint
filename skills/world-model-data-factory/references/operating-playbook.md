# Operating playbook: World model data factory

## Role in the blueprint

Role: convert real, simulated, and synthetic observations into model-ready evidence with repeatable quality gates.
...
2. **Evidence**: list the records already available and the records that are required by `schemas/dataset-manifest.schema.json`.
...
```bash
scripts/wmb new-manifest --schema schemas/dataset-manifest.schema.json --output artifacts/world-model-data-factory.json
scripts/wmb validate-manifest --schema schemas/dataset-manifest.schema.json --manifest artifacts/world-model-data-factory.json
