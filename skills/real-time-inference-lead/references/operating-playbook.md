# Operating playbook: Real-time inference lead

## Role in the blueprint

Role: make the adapted world model usable in batch, simulator, shadow, advisory, and real-time workflows with explicit acceleration controls.
...
2. **Evidence**: list the records already available and the records that are required by `schemas/inference-service-manifest.schema.json`.
...
```bash
scripts/wmb new-manifest --schema schemas/inference-service-manifest.schema.json --output artifacts/real-time-inference-lead.json
scripts/wmb validate-manifest --schema schemas/inference-service-manifest.schema.json --manifest artifacts/real-time-inference-lead.json
