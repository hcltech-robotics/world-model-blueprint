# Operating playbook: Cosmos post-training lead

## Role in the blueprint

Role: turn model-ready evidence into a governed Cosmos-class adaptation plan that can run and be served.
...
2. **Evidence**: list the records already available and the records that are required by `schemas/training-run-manifest.schema.json`.
...
```bash
scripts/wmb new-manifest --schema schemas/training-run-manifest.schema.json --output artifacts/cosmos-post-training-lead.json
scripts/wmb validate-manifest --schema schemas/training-run-manifest.schema.json --manifest artifacts/cosmos-post-training-lead.json
