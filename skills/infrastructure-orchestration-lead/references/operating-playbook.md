# Operating playbook: Infrastructure orchestration lead

## Role in the blueprint

Role: make training, evaluation, simulation, and inference executable on the right platform with a documented launch contract.
...
2. **Evidence**: list the records already available and the records that are required by `schemas/run-request.schema.json`.
...
```bash
scripts/wmb new-manifest --schema schemas/run-request.schema.json --output artifacts/infrastructure-orchestration-lead.json
scripts/wmb validate-manifest --schema schemas/run-request.schema.json --manifest artifacts/infrastructure-orchestration-lead.json
